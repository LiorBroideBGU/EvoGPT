...
class ChromosomesGenerator:
    ...
    async def _enhance_coverage(self, cfg, temperature, coverage_metrics, missed_branches, generated_unit_test,
                                thread_number, javafiles_dir, output_path: Path):
        """
        ...
        generated_enhancement = await test_enhancements.generation_repair_loop(coverage_metrics, missed_branches,
                                                                               thread_number=thread_number,
                                                                               output_path=output_path)
        if not generated_enhancement:
            self.logger.debug(f"Thread-{thread_number} coverage is good enough; skipping enhancement.")
            return generated_unit_test

        enhanced_unit_test = read_java_file_as_string(javafiles_dir / f'{self.class_name}EnhancedTest.java')
        return merge_java_unit_tests(generated_unit_test, enhanced_unit_test, f'{self.class_name}Test')

    async def threaded_generation(
            self, thread_number: int, temperature: float, max_retries: int = 3, output_path: Path = Path.cwd()):
        import sys as _sys

        for attempt in range(1, max_retries + 1):
            try:
                self.logger.debug(
                    f"Thread-{thread_number} starting (attempt {attempt}/{max_retries}) with temperature={temperature}")
                cfg = get_config()
                ...
                generated_running_test = await unit_test_generator.generation_repair_loop(
                    java_file_path=self.source_code_path, project_id=self.project_name, thread_number=thread_number,
                    output_path=output_path)
                ...
                # Coverage
                self.logger.debug(
                    f"Successfully generated a test for Thread-{thread_number}, verifying code coverage...")
                ...
                jcc = JavaCodeCoverage(java_files_dir=javafiles_dir, test_class=self.class_name,
                                       project_name=self.project_name, thread_id=thread_number)
                ...
                generated_unit_test = read_java_file_as_string(generated_test_path)
                if coverage_ok and coverage_xml_path.exists():
                    coverage_metrics, missed_branches = jcc.parse_jacoco_xml(coverage_xml_path)
                    final_unit_test = await self._enhance_coverage(
                        cfg=cfg, temperature=temperature, coverage_metrics=coverage_metrics, missed_branches=missed_branches,
                        generated_unit_test=generated_unit_test, thread_number=thread_number, javafiles_dir=javafiles_dir, output_path=output_path)

                else:
                    self.logger.debug(
                        f"Thread-{thread_number} coverage report failed; skipping enhancement, using base test")
                    final_unit_test = generated_unit_test

                # Cleanup
                delete_paths(
                    [generated_test_path, javafiles_dir / f'{self.class_name}EnhancedTest.java', classfiles_dir])

                # Save final merged test
                save_code(final_unit_test, generated_test_path)
                with self.lock:
                    chromosome = Chromosome(path=javafiles_dir, thread_id=thread_number)
                ...
                if attempt >= max_retries:
                    print(f"[Thread-{thread_number}] All {max_retries} attempts exhausted for {self.class_name}",
                          file=_sys.stderr)
    ...
    def save_chromosome_code(self, iteration: int, offspring1_code: str, offspring2_code: str, results_dir: str):
        base_path = Path(results_dir) / "unit_tests" / self.project_name / self.class_name / "offsprings" / str(
            iteration)
    ...
    async def _inject_llm_tests(self, best_chromosome: Chromosome, output_path: Path):
        ...
        self.logger.debug(
            f"Branch: {best_chromosome.branch_coverage:.1f}%, Line: {best_chromosome.line_coverage:.1f}%, Mutation: {best_chromosome.mutation_score:.1f}%")
    ...
    async def evolution_generation(self, max_generations=25, time_limit_seconds=None, output_path: Path = Path.cwd()):
        ...
        if time_limit_seconds:
            self.logger.debug(
                f"Starting Evolution: time limit {time_limit_seconds}s (max {max_generations} offspring pairs)")
        else:
        ...
        if cfg.LLM_INJECTION_ENABLED:
            self.logger.debug(
                f"LLM Injection: ENABLED (threshold={cfg.STAGNATION_THRESHOLD}, max={cfg.MAX_INJECTIONS}, agents=5)")
        else:
        ...
        while offspring_pairs_generated < max_generations:
            if time_limit_seconds and (time.time() - evolution_start_time) >= time_limit_seconds:
                self.logger.debug(
                    f"Time limit ({time_limit_seconds}s) reached after {offspring_pairs_generated} offspring pairs.")
                break
            if max(self.chromosomes, key=lambda c: c.fitness_score).fitness_score == float(100):
                self.logger.debug(
                    f"Perfect fitness (100.0) achieved after {offspring_pairs_generated} offspring pairs!")
            ...
            try:
                if random.random() < self.crossover_probability:
                    ...
                    offspring1_code, offspring2_code = read_java_file_as_string(
                        parent1.test_file_path), read_java_file_as_string(parent2.test_file_path)
                self.save_chromosome_code(offspring_pairs_generated, offspring1_code, offspring2_code, str(output_path))
                base_path = output_path / "unit_tests" / self.project_name / self.class_name / "offsprings" / str(
                    offspring_pairs_generated)
                ...
                if cfg.MUTATION_STRATEGY == 'llm':
                    ...
                    raise ValueError(
                        f"Unknown MUTATION_STRATEGY: '{cfg.MUTATION_STRATEGY}'. Expected 'llm' or 'programmatic'.")
                ...
                if best_offspring_fitness > best_parent_fitness or (
                        best_offspring_fitness == best_parent_fitness and offspring_code_length <= parents_code_length):
                ...
                if (cfg.LLM_INJECTION_ENABLED and
                    ...
                    self.logger.debug(
                        f"Triggering CodaMosa-style LLM injection #{injection_count + 1}/{cfg.MAX_INJECTIONS}...")
                    self.logger.debug(f"{'=' * 60}")
                ...
                if offspring_pairs_generated % 5 == 0:
                    ...
                    self.logger.debug(
                        f"Offspring pairs: {offspring_pairs_generated}/{max_generations} - Best: {best_current.fitness_score:.3f}, Avg: {avg_fitness:.3f}, Pop size: {len(self.chromosomes)}{stagnation_info}")
    ...
    def _save_best_and_cleanup(self, best_chromosome, output_path: Path | None = None):
        ...
        import shutil
        cfg = get_config()
        effective_output = Path(output_path) if output_path is not None else (
            Path(cfg.results_dir) if cfg.results_dir else Path.cwd())
...
if __name__ == '__main__':
    temperatures = [0.25]
    threads = []
    chromosomes_generator = ChromosomesGenerator(project_name='gson',
                                                 source_code_path=r"benchmarks/gson/src/main/java/com/google/gson/JsonArray.java")
    for i, temp in enumerate(temperatures):
